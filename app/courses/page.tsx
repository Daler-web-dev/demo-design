...
export default function Courses() {
  ...
  const filteredCourses = COURSES.filter(course => {
    ...
    const matchesSearch = course.title[lang].toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="bg-white min-h-screen pt-48 pb-32">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-24">
          <h1 className="text-6xl md:text-[10rem] font-display font-black tracking-tighter leading-none mb-12">CHOOSE <br />YOUR PATH.</h1>
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-8 py-3 rounded-2xl text-sm font-black uppercase tracking-widest transition-all ${
                    filter === cat.value ? 'bg-brand-900 text-white' : 'bg-brand-50 text-brand-400 hover:bg-brand-100'
                  }`}
                >
                  {cat.name}
                </button>
          ...
          {filteredCourses.map((course) => (
            <Link href={`/courses/${course.id}`} key={course.id} className="group block">
              <div className="relative aspect-[4/5] rounded-[3rem] overflow-hidden mb-8 shadow-xl">
                <img src={course.image} className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-700" alt={course.title[lang]} />
                <div className="absolute inset-0 bg-black/20 group-hover:bg-black/0 transition-all"></div>
              </div>
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-xs font-black text-accent uppercase tracking-[0.2em] mb-3">{course.category} • {course.level}</div>
                  <h3 className="text-4xl font-display font-black leading-tight mb-4 group-hover:text-brand-900">{course.title[lang]}</h3>
                  <div className="text-2xl font-black text-brand-900/40">{course.price[lang]}</div>
                </div>
