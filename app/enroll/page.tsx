...
export default function Enroll() {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const branch =
			(form.querySelector('[name="branch"]') as HTMLSelectElement)
				?.value ?? "";
		try {
			...
			<div className="min-h-[80vh] flex items-center justify-center p-4">
				<div className="max-w-md w-full text-center bg-white p-12 rounded-[3rem] shadow-2xl border border-gray-50 animate-in zoom-in duration-500">
					...
					<h2 className="text-3xl font-display font-bold text-brand-900 mb-4">
						{t.enrollForm.success}
					</h2>
					<p className="text-gray-500 mb-10">
						{t.enrollForm.successDetails}
					</p>
					<Link
						href="/"
		...
		<div className="min-h-[90vh] py-16 px-4 flex flex-col items-center justify-center bg-gray-50">
			<div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-12 bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-gray-100">
				<div className="p-10 md:p-16 flex flex-col justify-center">
					<h1 className="text-4xl md:text-5xl font-display font-bold text-brand-900 mb-6">
						{t.enrollForm.title}
					</h1>
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div>
							...
							<select
								name="branch"
								required
								className="w-full bg-gray-50 border-gray-100 rounded-2xl py-4 px-6 focus:ring-2 focus:ring-brand-500 transition-all text-brand-900 outline-none appearance-none"
							>
								<option value="">
									{t.enrollForm.branchPlaceholder}
								</option>
								{BRANCHES.map((b) => (
						...
						<p className="text-center text-sm text-gray-500 leading-relaxed pt-2">
							...
							<Link
								href="/privacy"
								className="text-brand-600 font-bold hover:underline underline-offset-2"
							>
				...
				<div className="hidden lg:block relative">
					<img
						src="https://picsum.photos/seed/enroll1/800/1000"
						alt={t.enrollForm.heroAlt}
					...
					<div className="absolute bottom-12 left-12 right-12 bg-white/90 backdrop-blur p-8 rounded-3xl">
						<div className="flex items-center gap-4 mb-4">
							<div className="w-12 h-12 bg-brand-900 rounded-full flex items-center justify-center text-white font-bold text-xl">
								&quot;
							</div>
							<div className="font-bold text-brand-900">
								{t.enrollForm.quoteAuthor}
							</div>
						</div>
